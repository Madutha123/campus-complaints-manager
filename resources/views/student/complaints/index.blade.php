...
<div class="space-y-8">
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
            <h1 class="page-heading">My Complaints</h1>
            <p class="page-subheading">View and track the status of your submitted issues.</p>
        </div>
    ...
    <div class="filter-bar">
        <form method="GET" action="{{ route('student.complaints.index') }}" class="flex flex-col sm:flex-row items-end gap-4 w-full sm:w-auto">
            ...
            <div class="flex gap-2 w-full sm:w-auto">
                ...
                @if($status)
                    <a href="{{ route('student.complaints.index') }}" class="btn-secondary flex-1 sm:flex-none">Clear</a>
                @endif
            </div>
        </form>
    </div>
                ...
                <tbody class="table-body">
                    @forelse ($complaints as $complaint)
                        <tr class="table-row table-row-accent">
                            ...
                            <td class="table-cell text-zinc-600 dark:text-zinc-400">{{ $complaint->category?->name ?? 'Uncategorized' }}</td>
                            ...
                            <td class="table-cell text-right">
                                <a href="{{ route('student.complaints.show', $complaint) }}" class="btn-ghost text-xs">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="table-cell text-center text-zinc-500 dark:text-zinc-400 py-16">No complaints found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
